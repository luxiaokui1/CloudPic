...
//            }
    ...
    public BaseResponse<PictureTagCategory> listPictureTagCategory() {
        ...
        List<String> tagList = Arrays.asList("Trending", "Funny", "Lifestyle", "HD", "Art", "Campus", "Background", "Resume", "Creative");
        List<String> categoryList = Arrays.asList("Template", "E-commerce", "Sticker", "Material", "Poster");
    ...
    /**
     * AI 分析图片（对已上传的图片进行 AI 分析，自动填充标签、描述、分类）
     */
    @PostMapping("/ai/analyze")
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<PictureAiAnalyzeResponse> aiAnalyzePicture(@RequestBody PictureAiAnalyzeRequest request) {
        ThrowUtils.throwIf(request == null || request.getPictureId() == null || request.getPictureId() <= 0, ErrorCode.PARAMS_ERROR);
        Long pictureId = request.getPictureId();
        // 查询图片
        Picture picture = pictureService.getById(pictureId);
        ThrowUtils.throwIf(picture == null, ErrorCode.NOT_FOUND_ERROR, "Picture not found");
        // 获取图片 key 和格式
        String imageKey = picture.getUrl();
        String mediaType = "image/" + (StrUtil.isNotBlank(picture.getPicFormat())
                ? picture.getPicFormat().toLowerCase() : "jpeg");
        // 调用 Claude AI 分析
        cn.hutool.json.JSONObject aiResult = claudeAiManager.analyzePicture(imageKey, mediaType);
        ThrowUtils.throwIf(aiResult == null, ErrorCode.OPERATION_ERROR, "AI analysis failed");
        // 构造响应
        PictureAiAnalyzeResponse response = new PictureAiAnalyzeResponse();
        if (aiResult.containsKey("tags")) {
            response.setTags(aiResult.getJSONArray("tags").toList(String.class));
        }
        if (aiResult.containsKey("description")) {
            response.setDescription(aiResult.getStr("description"));
        }
        if (aiResult.containsKey("category")) {
            response.setCategory(aiResult.getStr("category"));
        }
        // 更新图片信息到数据库
        Picture updatePicture = new Picture();
        updatePicture.setId(pictureId);
        if (response.getTags() != null) {
            updatePicture.setTags(JSONUtil.toJsonStr(response.getTags()));
        }
        if (StrUtil.isNotBlank(response.getDescription())) {
            updatePicture.setIntroduction(response.getDescription());
        }
        if (StrUtil.isNotBlank(response.getCategory())) {
            updatePicture.setCategory(response.getCategory());
        }
