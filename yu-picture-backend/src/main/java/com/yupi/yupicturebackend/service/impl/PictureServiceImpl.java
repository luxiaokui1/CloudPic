...
import com.yupi.yupicturebackend.manager.ClaudeAiManager;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    @Resource
    private ClaudeAiManager claudeAiManager;
    ...
    public void validPicture(Picture picture) {
        ...
        if (StrUtil.isNotBlank(introduction)) {
            ThrowUtils.throwIf(introduction.length() > 800, ErrorCode.PARAMS_ERROR, "Description too long");
        }
    }
...
//            }
            ...
            if (space.getTotalCount() >= space.getMaxCount()) {
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "Space picture count limit exceeded");
            }
            if (space.getTotalSize() >= space.getMaxSize()) {
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "Space storage limit exceeded");
            }
        }
        // 判断是新增还是删除
        Long pictureId = null;
...
//        picture.setPicColor(uploadPictureResult.getPicColor());
        ...
        // Claude AI 图片审核和分析（仅在用户未手动提供标签/描述/分类时执行）
        try {
            String imageKey = uploadPictureResult.getUrl();
            String mediaType = "image/" + (StrUtil.isNotBlank(uploadPictureResult.getPicFormat())
                    ? uploadPictureResult.getPicFormat().toLowerCase() : "jpeg");
            // AI 内容审核
            boolean isSafe = claudeAiManager.moderatePicture(imageKey, mediaType);
            if (!isSafe) {
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "Image content violates policy, blocked by AI moderation");
            }
            // AI 分析图片，自动填充标签、描述、分类（仅在用户未手动提供时）
            boolean needAiAnalysis = StrUtil.isBlank(picture.getTags())
                    && StrUtil.isBlank(picture.getIntroduction())
                    && StrUtil.isBlank(picture.getCategory());
            if (needAiAnalysis) {
                cn.hutool.json.JSONObject aiResult = claudeAiManager.analyzePicture(imageKey, mediaType);
                if (aiResult != null) {
                    if (aiResult.containsKey("tags")) {
                        picture.setTags(JSONUtil.toJsonStr(aiResult.getJSONArray("tags")));
                    }
                    if (aiResult.containsKey("description")) {
                        picture.setIntroduction(aiResult.getStr("description"));
                    }
                    if (aiResult.containsKey("category")) {
                        picture.setCategory(aiResult.getStr("category"));
                    }
                }
            }
            ...
            log.warn("Claude AI 分析图片失败，不影响上传流程", e);
        }
        // 操作数据库
        ...
        transactionTemplate.execute(status -> {
            ...
            ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR, "Picture upload failed, database operation error");
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "Failed to get page");
        }
        // 解析内容
        Element div = document.getElementsByClass("dgControl").first();
        if (ObjUtil.isEmpty(div)) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "Failed to get elements");
        }
...
//        checkPictureAuth(loginUser, oldPicture);
    ...
    public List<PictureVO> searchPictureByColor(Long spaceId, String picColor, User loginUser) {
        ...
        if (!space.getUserId().equals(loginUser.getId())) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "No space access permission");
        }
    ...
    public CreateOutPaintingTaskResponse createPictureOutPaintingTask(CreatePictureOutPaintingTaskRequest createPictureOutPaintingTaskRequest, User loginUser) {
        ...
        Picture picture = Optional.ofNullable(this.getById(pictureId))
                .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND_ERROR, "Picture not found"));
        // 校验权限，已经改为使用注解鉴权
